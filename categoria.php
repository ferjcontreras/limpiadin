...
<html lang="es" dir="ltr">
  <head>
    ...
    <script type="text/javascript">
      function VerCarrito(){
        ...
        if (UserID != ''){
          document.forms['Categoria'].action = 'ver_carrito.php';
      ...
      function SumarTotalCarrito() {
        document.forms['Categoria'].total_carrito.value = parseInt(document.forms['Categoria'].total_carrito.value) + 1;
        document.getElementById("numero_carrito").innerHTML = document.forms['Categoria'].total_carrito.value;
      }
  ...
  <body>
    ...
    <form name="Categoria" action="index.php" method="post">
      <input type="hidden" name="total_carrito" value="<?php echo $cantidad_carrito ?>">
      <?php include_once("header.php"); ?>
      <div class="cuerpo">
        <div class="container">
          <?php
            $sqlqry = "SELECT ID, Nombre FROM Categoria WHERE ID = '$categoria';";
          ...
          ?>
          <p id="nombre_categoria"><?php echo $DBarr[1] ?></p>
          <div class='row' style='display: flex; justify-content: space-between'>
            <?php
              $sqlqry = "SELECT ID, Nombre, Detalle, Precio, Foto FROM Producto WHERE IDCategoria = '$categoria' AND Disponible = 1 ORDER BY Nombre;";
              $DBres = mysqli_query($db, $sqlqry);
              if (mysqli_errno($db)) {
                echo "Error en consulta: $sqlqry";
              }
