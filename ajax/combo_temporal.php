<?php
    ...
    $sqlqry = "SELECT DetalleComboTemp.Cantidad, Producto.Nombre, Producto.Precio, DetalleComboTemp.IDProducto FROM Producto INNER JOIN DetalleComboTemp ON Producto.ID = DetalleComboTemp.IDProducto ORDER BY Producto.Nombre";
...
?>
      <div class="item_combo">
        <div class="row">
          ...
          <div class="col-sm-2">
            <input type="button" class="boton_eliminar" name="" value="X" style="margin-top:0.5em;" onclick="javascript:EliminarProductoTemporal(<?php echo $IDProducto[$i] ?>)">
          </div>
        </div>
      </div>
<?php
